...
def encoderPress(counter):
    ...
    if motorOn:
        ...
        motorOn = True
        pwm.start(50)
        pwm.ChangeFrequency((expected_rpm / 25) if expected_rpm is not None else 10)
...
def IRCallback(IRPin):
    global irCallbackAlreadyRunning, avg_rpm
    if not irCallbackAlreadyRunning:
        ...
        print(f"Raw RPM: {rpm}, Avg_RPM: {avg_rpm}, Expected_RPM: {expected_rpm}")
        irCallbackAlreadyRunning = False
...
while True:
    ...
    while motorOn:
        ...
        if clkState != lastClkState:
            ...
            pwm.ChangeFrequency((expected_rpm / 25))
